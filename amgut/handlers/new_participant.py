...
class NewParticipantHandler(BaseHandler):
    ...
    def post(self):
        ...
        if participant_name in participants:
            ...
            self.redirect(module_locale['SITEBASE'] + "/authed/portal/?errmsg=%s" % errmsg)
        ...
        if is_juvenile == 'on':
            ...
            if not is_exception:
                ...
                self.redirect(module_locale['SITEBASE'] + "/authed/portal/?errmsg=%s" % alert_message)

        self.redirect(module_locale['SITEBASE'] + "/authed/survey_main/")
